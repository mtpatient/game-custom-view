...
export default {
  ...
  methods: {
    ...
    handleToDetail() {
      ...
      const page = this.$router.resolve({
        path: '/post-detail/' + this.msg.post_id,
      })
      window.open(page.href, '_blank')
    },
    GetCommentDetail() {
      const id = this.msg.comment.parent_id !== 0 ? this.msg.comment.parent_id : this.msg.comment.id
  ...
  <div class="header">
    <div class="header-section">
      <div class="info-item pointer" @click="handleToUser">
        <div class="info-avatar">
          <el-image :src="msg.avatar" class="info-avatar-img"></el-image>
        </div>
        <div>
          <div class="name text-overflow0ellipsis-1">
            {{ msg.username }}
            <span class="type">点赞了我的{{ msg.comment === null ? '帖子' : '评论'}}</span>
          </div>
          <span class="time flex-vertical-center">{{ msg.create_time }}</span>
        </div>
      </div>
      <el-button style="display: inline-block" type="text" @click="handleToDetail">查看原帖</el-button>
    </div>
    <div class="item-content">
      <div style="margin-bottom: 12px" v-html="msg.content"></div>
      <div class="comment-section flex">
        <div class="comment-content pointer" v-if="msg.comment === null" @click="handleToDetail">
          <div>帖子：</div>
          <div>{{ msg.post.title }}</div>
        </div>
        <div class="comment-content pointer" v-else @click="GetCommentDetail">
          <div>评论：</div>
          <div v-html="msg.comment.content"></div>
        </div>
      </div>
    </div>
    <el-dialog :visible.sync="dialogVisible"
      ...
      <div class="dialog-header">
        <h1>{{ commentDetail.child === null ? 0 : commentDetail.child.length }} 条回复</h1>
      </div>
